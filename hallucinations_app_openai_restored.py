...
load_dotenv()
st.write("🔐 Render sees OPENAI_API_KEY:", os.getenv("OPENAI_API_KEY") is not None)
...
async def call_grok(prompt):
    try:
        grok_prompt = f"""You are Grok (simulated): witty, direct, and accurate.
...
{prompt}"""
        ...
        return ("Grok (simulated)", f"[Grok simulation error: {str(e)}]")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("https://openrouter.ai/api/v1/chat/completions", json=payload, headers=headers) as response:
                ...
                if "choices" not in result:
                    return ("OpenRouter", f"[OpenRouter error: {result.get('error', 'unexpected format')}]")
...
model_list = ["GPT-4o", "Claude 3 Haiku", "Gemini 1.5 Pro", "Grok", "OpenRouter"]
if suggested_model:
